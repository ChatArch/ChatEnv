...
def _group_items(group: click.Group) -> list[tuple[str, str | click.Command]]:
    ...
    return [(name, group.commands[name]) for name in group.list_commands(ctx)]


def render_cli_tree(root: click.Group | None = None) -> str:
    """Render the registered Click command tree for `chatenv --tree`."""
    if root is None:
        root = cli
    lines = [f"chatenv{_command_signature(root)}  # {_short_help(root)}"]
    ...
    def walk(items: list[tuple[str, str | click.Command]], prefix: str = "") -> None:
        for index, (name, item) in enumerate(items):
            ...
            if isinstance(item, str):
                ...
                continue
            lines.append(f"{prefix}{connector}{name}{_command_signature(item)}  # {_short_help(item)}")
